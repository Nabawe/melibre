...
// TO-DO the string values could use a regex to check for last the /, file path, name, etc
// ? Is there a better way to Type .i, .#init(), #dataChecks?
class RAMBox {
    /**
    ...
    /* ? How to Type catch ( err ). Catch clause variable type annotation must be 'any' or 'unknown' if specified.ts(1196) */
    /**
    ...
    // ? A way to tell that flags keys must be all caps and underscores? regex?
    // * Both Parameters are optional
    #dataChecks( flags: { [ key: string ]: boolean }, data = this.i ) {
     ...
     */
    async m_fileGetAll(): Promise<object[] | Error> {
        try {
     ...
     * @returns {object}JSON formmated JavaScript object.
     */
    m_fileGetAllSync(): Promise<object[] | Error> {
        try {
